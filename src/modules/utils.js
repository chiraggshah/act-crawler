...
export const logDebug =
  NODE_ENV === "production" ? () => {} : message => log(message, "DEBUG");
...
export const normalizeUrl = (url, keepFragment) => {
  ...
  return `${urlObj.protocol
    .trim()
    .toLowerCase()}://${urlObj.host.trim().toLowerCase()}${path.trim()}${
...
export const getValueOrUndefined = async key => {
  const value = await Apify.getValue(key).catch(() => undefined);
...
export const setValue = async ({ key, body, contentType, recursion = 0 }) => {
  ...
  const promise = Apify.setValue(key, body, opts)
    .then(() => {
    ...
    .catch(err => {
      if (recursion > SET_VALUE_MAX_REPEATS) {
        logError(
          `Cannot set value ${key} in iteration ${recursion}, giving up`,
          err
        );
        return;
      }
      ...
      logError(
        `Cannot set value ${key} in iteration ${recursion}, trying once again`,
        err
      );
      setTimeout(
        () => setValue({ key, body, contentType, recursion: recursion + 1 }),
        15 * 1000
      );
    });
...
export const checkParamOrThrow = (value, name, type, message) => {
  if (!message)
    message = `Parameter "${name}" of type ${type} must be provided`;

  const allowedTypes = parseType(type);

  const allowsBuffer = allowedTypes.filter(item => item.type === "Buffer")
    .length;
  const allowsPromise = allowedTypes.filter(item => item.type === "Promise")
    .length;
