 ...
 */
export const waitForBody = async page =>
  page.waitFor("body", { timeout: 15000 }); // @TODO put this timeout into configuration
...
const exposeMethod = async (page, method, name) => {
  ...
  return page.exposeFunction(exposedName, method).then(() =>
    page.evaluate(
      (passedExposedName, passedName) => {
        console.log(
          `Exposing window.${passedExposedName}() as context.${passedName}()`
        );
        window.APIFY_CONTEXT = window.APIFY_CONTEXT || {};
        ...
        window.APIFY_CONTEXT.pendingPromises =
          window.APIFY_CONTEXT.pendingPromises || {};
...
export const decorateEnqueuePage = async (page, interceptRequestStr) => {
  return page.evaluate(
    (passedInterceptRequestStr, allowedFields) => {
      console.log("Decorating context.enqueuePage()");

      if (!passedInterceptRequestStr)
        passedInterceptRequestStr = "function (ctx, req) { return req; }";
      ...
      if (typeof interceptRequest !== "function")
        throw new Error("InterceptRequest must be a function string!");
      ...
      context.enqueuePage = async (requestOpts, clickedElement = null) => {
        const newRequest = await context.newRequest(
          pick(requestOpts, allowedFields)
        );
        ...
        const interceptedRequest = interceptRequest(
...
export const executePageFunction = async (page, crawlerConfig) => {
  return page.evaluate(passedCrawlerConfig => {
    ...
    context.willFinishLater = () => {
      ...
      // Create timeout.
      const remainsMillis =
        passedCrawlerConfig.pageFunctionTimeout - (new Date() - startedAt);
    ...
    try {
      ...
      return Promise.all(Object.values(context.pendingPromises)) // Pending calls to exposed methods like enqueuePage() ...
        .then(
          () => willFinishLaterPromise || Promise.resolve(pageFunctionResult)
        )
        .then(result => {
...
export const clickClickables = async (page, clickableElementsSelector) => {
  return page.evaluate(passedClickableElementsSelector => {
    ...
    document.querySelectorAll(passedClickableElementsSelector).forEach(el => {
      const url = el.href;
 ...
 */
export const scrollTo = (page, xpos, ypos) =>
  page.evaluate((x, y) => window.scrollTo(x, y), xpos, ypos);

/**
 * Method returns info about page scroll
 */
export const getPageScrollInfo = page =>
  page.evaluate(() => {
    return {
...
export const infiniteScroll = async (page, maxHeight) => {
  ...
  try {
    ...
    while (true) {
      ...
      logDebug(
        `Infinite scroll stats (${stringifyScrollInfo(
          scrollInfo
        )} resourcesStats=${JSON.stringify(resourcesStats)}).`
      );
      ...
      if (pendingRequestsCount === 0) {
        ...
        if (
          ...
          Math.min(scrollInfo.scrollHeight, maxHeight)
        )
          break;
    ...
    logInfo(
      `Infinite scroll finished (${stringifyScrollInfo(
        scrollInfo
      )} resourcesStats=${JSON.stringify(resourcesStats)})`
    );
  } catch (err) {
