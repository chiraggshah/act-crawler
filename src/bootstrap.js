...
import LocalPageQueue, {
  STATE_KEY as PAGE_QUEUE_STATE_KEY,
} from "./modules/local_page_queue";
import LocalSequentialStore, {
...
process.on('exit', code => {
  ...
  exec(`curl -X PUT ${BASE_URL}/suppliers/${SUPPLIER_ID}/crawler_status/${CRAWLER_ID} -d status=${status} apify_request_token=${APIFY_REQUEST_TOKEN}`);
...
const fetchInput = async () => {
  ...
  const crawler = input.crawlerId
    ? await Apify.client.crawlers.getCrawlerSettings({
...
Apify.main(async () => {
  ...
  crawler.on(EVENT_REQUEST, request => {
    ...
    if (
      input.maxCrawlDepth &&
      request.type !== REQUEST_TYPES.USER_ENQUEUED &&
      request.depth > input.maxCrawlDepth
    ) {
      logDebug(
        `Not enqueuing ${request.url}, type = ${
          request.type
        }, max depth reached`
      );
      return;
    ...
    if (!request.willLoad) {
      logDebug(
        `Not enqueuing ${request.url}, type = ${request.type}, willLoad = false`
      );
      return;
    }
  ...
  crawler.on(EVENT_SNAPSHOT, ({ url, html, screenshot }) => {
    ...
    setValue({
      key: `SNAPSHOT-${filename}.html`,
      body: html,
      contentType: "text/html",
    });
    setValue({
      key: `SNAPSHOT-${filename}.jpg`,
      body: screenshot,
  ...
  const promiseProducer = () => {
    ...
    if (
      urlList &&
      !isUrlListDone &&
      (!input.maxCrawledPages ||
        pageQueue.getPageCount() < input.maxCrawledPages)
    ) {
    ...
    return new Promise(async resolve => {
      ...
      try {
        ...
        if (request.retryCount === input.maxPageRetryCount) {
          logDebug(
            `Load failed too many times, giving up (request.id: ${
              request.id
